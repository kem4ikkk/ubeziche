...
def i_ghost(im, d, dk):
    ...
    d.pieslice([140, 108, 372, 340], 180, 360)            # купол
    d.rectangle([140, 224, 372, 392])                     # тело
    n = 4
    x0 = 140.0; w = (372 - 140) / n
    for i in range(n):                                    # острые капли вниз
        xa = x0 + i * w
        poly(d, [(xa, 392), (xa + w, 392), (xa + w / 2, 440)])
    dk.ellipse([200, 232, 234, 292], fill=K)              # глаза (верт. овалы)
    dk.ellipse([278, 232, 312, 292], fill=K)
...
def i_factory(im, d, dk):
    # Завод с ПИЛООБРАЗНОЙ крышей (узнаваемый символ) + труба с дымом.
    d.rectangle([112, 250, 404, 420])                     # корпус
    n = 4
    x0 = 112.0; w = (404 - 112) / n
    for i in range(n):
        xa = x0 + i * w
        poly(d, [(xa, 250), (xa, 186), (xa + w, 250)])    # зубец крыши
    rrect(d, [300, 116, 338, 250], 4)                     # труба
    disc(d, 319, 96, 17); disc(d, 342, 76, 13)            # дым
    for x in (158, 230, 358):                             # окна (тонкие)
        dk.rectangle([x - 11, 320, x + 11, 378], fill=K)
