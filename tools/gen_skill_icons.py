...
def i_ghost(im, d, dk):
    # Классический призрак: купол + тело + ОСТРЫЙ зигзаг снизу (а не «колёса»).
    d.pieslice([140, 108, 372, 340], 180, 360, fill=W)    # купол
    d.rectangle([140, 224, 372, 392], fill=W)             # тело
    n = 4
    x0 = 140.0; w = (372 - 140) / n
    for i in range(n):                                    # острые капли вниз
        xa = x0 + i * w
        poly(d, [(xa, 392), (xa + w, 392), (xa + w / 2, 440)])
    dk.ellipse([210, 244, 236, 282], fill=K)              # МАЛЕНЬКИЕ глаза (больше белого)
    dk.ellipse([276, 244, 302, 282], fill=K)


def i_heart(im, d, dk):
    # Сплошное сердце из двух круглых долей + остриё (форма читается как сердце,
    # а не как треугольник ▽).
    disc(d, 196, 214, 84)                                 # левая доля
    disc(d, 316, 214, 84)                                 # правая доля
    poly(d, [(116, 234), (396, 234), (256, 420)])         # низ к острию
...
def i_factory(im, d, dk):
    # Завод: ПИЛООБРАЗНАЯ крыша (узнаваемый символ) + труба с дымом. Крупно и
    # просто, чтобы читалось при 26px: 3 зубца, 2 окна.
    d.rectangle([112, 256, 400, 422], fill=W)             # корпус
    n = 3
    x0 = 112.0; w = (400 - 112) / n
    for i in range(n):
        xa = x0 + i * w
        poly(d, [(xa, 256), (xa, 192), (xa + w, 256)])    # крупный зубец крыши
    rrect(d, [298, 126, 340, 256], 4)                     # труба
    disc(d, 319, 104, 19)                                 # дым
    for x in (168, 252):                                  # 2 окна (крупные)
        dk.rectangle([x - 15, 330, x + 15, 388], fill=K)
