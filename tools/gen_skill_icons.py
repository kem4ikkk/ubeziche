...
OW = 17                       # толщина чёрной обводки (в S-пикселях)
W = (255, 255, 255, 255)
K = (0, 0, 0, 255)
...
def i_ghost(im, d, dk):
    d.pieslice([138, 120, 374, 356], 180, 360)            # купол
    d.rectangle([138, 238, 374, 414])                     # тело
    for xc in (180, 256, 332):                            # волнистый низ
        poly(d, [(xc - 48, 410), (xc + 48, 410), (xc, 468)])
    dk.ellipse([192, 250, 236, 304], fill=K)              # глаза (чёрные)
    dk.ellipse([276, 250, 320, 304], fill=K)


def i_heart(im, d, dk):
    d.pieslice([130, 150, 268, 288], 180, 360)
    d.pieslice([244, 150, 382, 288], 180, 360)
    poly(d, [(140, 232), (372, 232), (256, 424)])
    stroke(dk, [(256, 210), (256, 300)], 26, K)           # «+» медицинский
    stroke(dk, [(212, 255), (300, 255)], 26, K)
...
def i_tower(im, d, dk):
    stroke(d, [(256, 156), (184, 424)], 20)               # ноги
    stroke(d, [(256, 156), (328, 424)], 20)
    for y in (236, 312, 388):                             # перемычки
        lx = 256 - (y - 156) * (72.0 / 268)
        rx = 256 + (y - 156) * (72.0 / 268)
        stroke(d, [(lx, y), (rx, y)], 14)
    disc(d, 256, 150, 18)                                 # антенна
    stroke(d, [(256, 150), (256, 104)], 12)
    for r in (54, 84):                                    # сигнал
        dk.arc([256 - r, 104 - r, 256 + r, 104 + r], 215, 325, fill=K, width=12)


def i_factory(im, d, dk):
    d.rectangle([124, 274, 404, 422])                     # корпус
    rrect(d, [150, 188, 196, 274], 4)                     # трубы
    rrect(d, [228, 168, 274, 274], 4)
    rrect(d, [306, 208, 352, 274], 4)
    for x in (160, 224, 288, 352):                        # окна
        dk.rectangle([x - 16, 320, x + 16, 372], fill=K)
...
def i_bricks(im, d, dk):
    d.rectangle([118, 176, 394, 392])
    for y in (248, 320):                                  # горизонт. шов
        dk.line([(118, y), (394, y)], fill=K, width=12)
    rows = [(176, 248, [256]), (248, 320, [187, 325]), (320, 392, [256])]
    for y0, y1, xs in rows:                               # вертик. швы (вразбежку)
        for x in xs:
            dk.line([(x, y0), (x, y1)], fill=K, width=12)
